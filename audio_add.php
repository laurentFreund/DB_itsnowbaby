...
|**		Description:				Audio files upload page                              **|
\**********************************************************************************************/
################################################################################################


	$current_menu = 2;//to display 'UPLOAD' menu as selected
	$title = utf8_encode("Audio add");//title for the window
	if(isset($_GET['id'])) $title = utf8_encode("Audio edit");
	include("header.php");
...
{	
	if(!isset($_POST['id']))//If it's a new media, we check it, move it, convert it, rename it, then add it to the database 
	{
		...
		else if (is_uploaded_file($_FILES['thefile']['tmp_name'])) //else if the upload was successfull
		{
			...
			// the list of allowed extensions for the cenverter or the application
			$allowedExtensions = array('caf','aiff','aac','gsm','aea','alaw','amr','apc','ape','au','dxa','mp2','mp3','oga','oss','ra','ogg','rm','tta','wav','voc','w64','wma'); 
			if(in_array($extensionFile,$allowedExtensions))// if the file's extention is OK, we can add it to our database
			{
				...
				$relative_link = "medias/audios/".$nameDestination ;// new file's name in his path
				...
				{
					//get duratation for length in the DB
					$movie = new ffmpeg_movie($link);
					...
					if($extensionFile != 'mp3'){
						...
						system(escapeshellcmd("ffmpeg -i ".$relative_link." -ar 44100 -ac 2 -ab 192k -f mp3 medias/audios/".$id."_".$title.".mp3"), $retval);
					}
					else{
						$retval = 0;
						$unlink = false;
					...
					if($retval == 0){
						...
						$query1 = sprintf("INSERT INTO inb_medias_1(id,mediatype,title,authors,themes,locations,persons,misc,genre,system,direct_links,time,upload_date,link_type,link,weight,extension,length,associated_text) 
						...
						if($request1)
						{
							/* symetric associated text: if audio1 set text2 as associated text, then text2 will have audio1 as associated audio */
							$query_aa = sprintf("UPDATE inb_medias_1 SET associated_audio='%d' WHERE id='%d'",$id,mysql_real_escape_string(htmlspecialchars($_POST['associated_text'])));
							...
							if($request_aa && !($error)){
								...
								if(!($error)){
									$success = true;
									$successText .= "The audio track has been registered into the database successfully.";
								}
							}
							else{
						...
						$errorText .= "An error has occurred while converting to mp3 : ".$retval."<br /> ";
						$error = true;
					}
				}
			}
	...
	else// this isn't a new media, one was edited 
	{
		...
		$query_aa = sprintf("UPDATE inb_medias_1 SET associated_audio='%d' WHERE id='%d'",$_POST['id'],mysql_real_escape_string(htmlspecialchars($_POST['associated_text'])));
		...
		if(!($request) || !($request_aa) || $error){
			$errorText .= "An error has occurred during the updating of the audio track (level 1.0) into the database: <br />".mysql_error()."<br /> ";
			$error = true;
		}
		update_media('audio');
...
if(isset($_GET['id'])){// if we are modifying a media, we get all the metadata from the database
	$query = sprintf("SELECT * FROM inb_medias_1 WHERE mediatype=1 AND link_type=1 AND id='%d'",$_GET['id']);
	$request = mysql_query($query);
	if(!($request))
	{
	...
	display_notification_box_and_title('an audio track');//we display de title and the nofication box
	?>
	
	<form action="audio_add.php" method="post" class="form" id="add_form" name="add_form" enctype="multipart/form-data" >
	
		<?php
		display_head_metadata('audio');// title, authors, date, associated text
		display_media_add_fields('audio');// file 
	...
	<?php 
	echo_options_add_value_init('audio'); //initialize some usefull variables
	//the javascript fonction for the submit
	echo_submit_function('Converting to mp3 ...',"'caf','aiff','aac','gsm','aea','alaw','amr','apc','ape','au','dxa','mp2','mp3','oga','oss','ra','ogg','rm','tta','wav','voc','w64','wma'");
	?>	

<?php if(isset($_GET['id'])) { //if are modifying an audio track
		?>
